...
class Ship(GImage):
    ...
    def __init__(self,x2,y2,wid,hei,sou):
        """
        Create a single ship instance
        """
        super().__init__(x=x2,y=y2,width=wid,height=hei,source=sou)


    # METHODS TO MOVE THE SHIP AND CHECK FOR COLLISIONS
    def moveship(self,input,ship):
        """
        Move ship to right if player types left arrow and move ship to left if
        player types right arrow
        """
        if input.is_key_down('left') and ship.x>=SHIP_WIDTH/2:
            ship.x -= SHIP_MOVEMENT
        if input.is_key_down('right') and ship.x<=(GAME_WIDTH-SHIP_WIDTH/2):
            ship.x += SHIP_MOVEMENT


    def shipcollision(self,bolt,ship):
        """
        Return True if ship is destroyed by a lazer bolt from aliens
        """
        if bolt.getv() < 0:
            if ship.contains((bolt.x-BOLT_WIDTH/2,bolt.y-BOLT_HEIGHT/2)):
                return True
            elif ship.contains((bolt.x+BOLT_WIDTH/2,bolt.y-BOLT_HEIGHT/2)):
                return True
            elif ship.contains((bolt.x-BOLT_WIDTH/2,bolt.y+BOLT_HEIGHT/2)):
                return True
            elif ship.contains((bolt.x+BOLT_WIDTH/2,bolt.y+BOLT_HEIGHT/2)):
...
class Alien(GImage):
    ...
    def __init__(self,x2,y2,wid,hei,s):
        """
        Create a single alien instance
        """
        super().__init__(x=x2,y=y2,width=wid,height=hei,source=s)


    # METHOD TO CHECK FOR COLLISION (IF DESIRED)
    def aliencollision(self,bolt,alien):
        """
        Return true if a alien is destroyed by lazer bold from ship
        """
        if alien.contains((bolt.x-BOLT_WIDTH/2,bolt.y-BOLT_HEIGHT/2)):
            return True
        elif alien.contains((bolt.x+BOLT_WIDTH/2,bolt.y-BOLT_HEIGHT/2)):
            return True
        elif alien.contains((bolt.x-BOLT_WIDTH/2,bolt.y+BOLT_HEIGHT/2)):
            return True
        elif alien.contains((bolt.x+BOLT_WIDTH/2,bolt.y+BOLT_HEIGHT/2)):
...
class Bolt(GRectangle):
    ...
    def getv(self):
        """
        Returns the velocity of the laser bolt.
        """
        return self._velocity


    #INITIALIZER TO SET THE VELOCITY
    def __init__(self, x, y, w, h, l, f, v):
        """
        Create a single instance of bolt
        """
        super().__init__(x=x, y=y, height=h, linecolor=l, fillcolor=f)
        if self.y==SHIP_BOTTOM+SHIP_HEIGHT/2:
            self._velocity=v
        else:
            self._velocity=-v
    # INITIALIZER TO SET THE VELOCITY
